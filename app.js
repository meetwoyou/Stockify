...
// Lets the app open + show products without internet, syncs when back online.
...
function switchPage(page) {
  ...
  // Auto-start scanner when opening Scanner page
  if (page === "scanner" && !scannerRunning) {
...
function handleScan(code) {
  ...
  if (product) {
    // Auto-stop and show product details instantly
    stopScanner();
    showScanModal(product);
  } else {
    // Unknown SKU → auto-open Add Product form with SKU pre-filled
    lastScannedSku = code;
    stopScanner();
    switchPage("products");
    openAddProductForm();
